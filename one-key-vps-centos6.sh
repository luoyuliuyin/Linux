...
#####---shadowsocks---#####;
yum install epel-release -y;yum update -y;yum install python-setuptools -y;easy_install pip;pip install shadowsocks;echo 'alias sss="tail -f /var/log/shadowsocks.log"' >> /etc/bashrc;echo -e 'ExecStart=/usr/bin/ssserver -p 444 -k pengfeixiong -m aes-256-cfb --workers 5 -d start' >> /etc/rc.local;
