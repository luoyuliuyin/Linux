...
yum update -y;yum install vim wget git -y;yum install libsodium-1.0.17-1.el7.x86_64.rpm -y;git clone -b manyuser https://github.com/shadowsocksr-backup/shadowsocksr.git;cd ~/shadowsocksr;bash initcfg.sh;echo 'alias log="tail -f /var/log/shadowsocksr.log"' >> /etc/bashrc;echo -e '[Unit]\nDescription=Shadowsocksr\n\n[Service]\nType=forking\nExecStart=/usr/bin/python /root/shadowsocksr/shadowsocks/server.py -p 4399 -k pengfeixiong -m chacha20 -O auth_sha1_v4 -o http_simple --workers 2 -d start\n\n[Install]\nWantedBy=multi-user.target' > /etc/systemd/system/shadowsocksr.service;systemctl enable shadowsocksr;
