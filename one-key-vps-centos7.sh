...
#####---shadowsocks---#####;
#yum update -y;yum install epel-release -y;wget ftp://ftp.pbone.net/mirror/download.fedora.redhat.com/pub/fedora/epel/7/x86_64/Packages/l/libsodium-1.0.17-1.el7.x86_64.rpm;yum install vim wget net-tools python-setuptools libsodium-1.0.17-1.el7.x86_64.rpm -y;easy_install pip;pip install shadowsocks;echo 'alias log="tail -f /var/log/shadowsocks.log"' >> /etc/bashrc;echo -e '[Unit]\nDescription=Shadowsocks\n\n[Service]\nType=forking\nExecStart=/usr/bin/ssserver -p 4399 -k pengfeixiong -m chacha20 --workers 2 -d start\n\n[Install]\nWantedBy=multi-user.target' > /etc/systemd/system/shadowsocks.service;systemctl enable shadowsocks;
yum update -y;yum install vim wget git -y;yum install libsodium-1.0.17-1.el7.x86_64.rpm -y;cd /opt;git clone -b manyuser https://github.com/shadowsocksr-backup/shadowsocksr.git;cd /opt/shadowsocksr;bash initcfg.sh;echo 'alias log="tail -f /var/log/shadowsocksr.log"' >> /etc/bashrc;echo -e '[Unit]\nDescription=Shadowsocksr\n\n[Service]\nType=forking\nExecStart=/usr/bin/python /opt/shadowsocksr/shadowsocks/server.py -p 4399 -k pengfeixiong -m chacha20 -O auth_sha1_v4 -o http_simple --workers 2 -d start\n\n[Install]\nWantedBy=multi-user.target' > /etc/systemd/system/shadowsocksr.service;systemctl enable shadowsocksr;
...
cd;rm -f one-key-vps-centos7.sh *.rpm *.log;reboot;
