...
#####---shadowsocks---#####;
yum update -y;yum install epel-release -y;yum install vim wget net-tools python-setuptools -y;easy_install pip;pip install shadowsocks;echo 'alias sss="tail -f /var/log/shadowsocks.log"' >> /etc/bashrc;echo -e '[Unit]\nDescription=Shadowsocks\n\n[Service]\nType=forking\nExecStart=/usr/bin/ssserver -p 443 -k dafeige -m rc4-md5 --workers 2 -d start\n\n[Install]\nWantedBy=multi-user.target' > /etc/systemd/system/shadowsocks.service;systemctl enable shadowsocks;
