...
#####---shadowsocks---#####;
yum update -y;yum install vim wget git -y;wget https://download-ib01.fedoraproject.org/pub/epel/7/x86_64/Packages/l/libsodium-1.0.18-1.el7.x86_64.rpm;yum install libsodium-1.0.18-1.el7.x86_64.rpm -y;cd /opt;git clone -b manyuser https://github.com/shadowsocksr-backup/shadowsocksr.git;cd /opt/shadowsocksr;bash initcfg.sh;echo 'alias log="tail -f /var/log/shadowsocksr.log"' >> /etc/bashrc;echo -e '[Unit]\nDescription=Shadowsocksr\n\n[Service]\nType=forking\nExecStart=/usr/bin/python /opt/shadowsocksr/shadowsocks/server.py -p 4399 -k pengfeixiong -m chacha20 -O auth_aes128_md5 -o tls1.2_ticket_auth --workers 2 -d start\n\n[Install]\nWantedBy=multi-user.target' > /etc/systemd/system/shadowsocksr.service;systemctl enable shadowsocksr;
